...
I have a CSV file which contains data on the spectral reflectance of post-dried soil samples between the wavelengths 350 nm and 2500 nm in increments of 1 nm.
...
```{r}
RefDry <- read.csv("SoilReflectanceDry_v01.csv")
```
...
Export the continuum removal data as a CSV file:
```{r}
write.csv(RefDryContinuumRemoved, file = "RefDry_ContinuumRemoved_v01.csv",
          row.names = F)
```
